...
import { isTeamAccount as isTeamAccountRule } from './teamAccount';
...
export async function loadCompanionProps(req, licenseKey) {
  ...
  // Passed to the sidebar so My Team and its children don't vanish on
  // these pages.
  const isTeamAccount = isTeamAccountRule(tokens, activeLicenses);
  if (user?.email) {
  ...
  return {
    ...
    me, licenses: activeLicenses, isIndividual, isTeamAccount,
  };
}

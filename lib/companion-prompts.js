...
import { REGISTERS, METHOD, GUARDRAILS } from './translation-prompts';
...
// The Translator is universal (not personalized to the caller's own
// profile): its target register is stated in the user message, not injected
// here, so it skips the profile/tertiary line entirely.
export function buildSystemPrompt({ tool, mode, profile, tertiary }) {
  if (tool === 'translator') {
    return [
      SOT_PREAMBLE,
      { type: 'text', text: `${SYSTEM_BASE.translator}\n\n${MODE_SYSTEM.translator[mode]}` },
    ];
  }

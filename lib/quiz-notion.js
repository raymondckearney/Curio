...
// how-what), Submission Date (date), Q01-Q23 (text — "ORIENTATION: chosen
// answer text", one per question, for spot-checking individual responses —
// zero-padded so they sort/display in numeric rather than alphabetical
// order in Notion), Tiebreaker Answer (text, only set when a tie-breaker
// was shown). The
...
export async function logQuizResponseToNotion({
  ...
  if (answers) {
    for (const [id, text] of Object.entries(answers)) {
      properties[`Q${String(id).padStart(2, '0')}`] = richText(text);
    }
  }

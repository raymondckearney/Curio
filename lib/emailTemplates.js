...
export const TRIGGER_OPTIONS = [
  ...
  { key: 'cron_weekly_tip',      label: 'Cron — weekly profile tip' },
  { key: 'assistant_access_request', label: 'Curio Assistant access request' },
...
export const TRIGGER_VARIABLES = {
  ...
  cron_weekly_tip:   ['name', 'email', 'profileLabel', 'profileTagline', 'tipNumber', 'tipHeadline', 'tipBody', 'unsubscribeUrl'],
  assistant_access_request: ['requesterName', 'requesterEmail', 'accountName', 'itemName', 'itemKind', 'reason', 'userQuery', 'adminUrl', 'ownerEmails'],
  manual:            [],
};
...
export const EMAIL_TEMPLATE_REGISTRY = [
  ...
  {
    key: 'weekly_tip',
    name: 'Weekly Profile Tip',
    description: 'Weekly cron: one of the recipient\'s 20 profile-specific tips, sent in order. Edit this template\'s HTML any time to change the card\'s look — nothing here needs a code deploy.',
    recipient: '{{email}}',
    trigger: 'cron_weekly_tip',
    trigger_label: 'Cron — weekly profile tip',
    schedule: 'Weekly cron, Mondays',
    send_type: 'automated',
    variables: ['name', 'email', 'profileLabel', 'profileTagline', 'tipNumber', 'tipHeadline', 'tipBody', 'unsubscribeUrl'],
    default_subject: 'Your MindPrint™ tip of the week',
    default_html_body: `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#F8FAFC;font-family:Helvetica,Arial,sans-serif">
<div style="max-width:560px;margin:32px auto">
  <p style="margin:0 0 20px;color:#0F172A;font-size:15px;line-height:1.7">Hi {{name}}, this is your <strong>Weekly MindPrint™ Tip</strong> — one small, specific way to work with your tertiary orientation instead of against it, picked for your own profile. New tip every Monday.</p>
  <p style="margin:0 0 24px"><a href="https://choosecurio.com" style="color:#059669;font-weight:700;font-size:14px;text-decoration:none">&larr; Visit choosecurio.com</a></p>
  <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#0F172A;border-radius:16px;overflow:hidden">
    <tr><td style="padding:36px 36px 28px">
      <table cellpadding="0" cellspacing="0" border="0"><tr>
        <td style="padding-right:8px"><div style="width:8px;height:8px;border-radius:50%;background:#34D399"></div></td>
        <td style="color:#6EE7B7;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;font-family:Helvetica,Arial,sans-serif">{{profileLabel}} &middot; Weekly Tip {{tipNumber}}</td>
      </tr></table>
      <p style="margin:20px 0 0;color:#fff;font-size:26px;line-height:1.25;font-weight:700;font-family:Georgia,'Times New Roman',serif">{{tipHeadline}}</p>
      <p style="margin:18px 0 26px;color:#CBD5E1;font-size:15px;line-height:1.7">{{tipBody}}</p>
      <table cellpadding="0" cellspacing="0" border="0"><tr>
        <td style="background:#6EE7B7;border-radius:99px;padding:7px 16px"><span style="color:#064E3B;font-size:12px;font-weight:700">{{profileTagline}}</span></td>
        <td style="padding-left:14px;color:#64748B;font-family:Georgia,serif;font-size:15px;font-style:italic">Curio.</td>
      </tr></table>
    </td></tr>
  </table>
  <p style="margin:20px 0 0;color:#94A3B8;font-size:12px;line-height:1.6;text-align:center"><a href="{{unsubscribeUrl}}" style="color:#94A3B8;text-decoration:underline">Unsubscribe from these tips</a></p>
</div>
</body></html>`,
  },
  {
    key: 'internal_access_request',
    name: 'Access Request (to Curio)',
    description: 'Sent to hello@choosecurio.com when someone asks the Curio Assistant for access to a tool or resource. Approve or dismiss it in Admin → Assistant.',
    recipient: 'hello@choosecurio.com',
    trigger: 'assistant_access_request',
    trigger_label: 'Curio Assistant access request',
    schedule: 'Immediate',
    send_type: 'automated',
    variables: ['requesterName', 'requesterEmail', 'accountName', 'itemName', 'itemKind', 'reason', 'userQuery', 'adminUrl'],
    default_subject: 'Access request: {{itemName}} for {{requesterName}} ({{accountName}})',
    default_html_body: emailWrap(`
    <p style="margin:0 0 20px;font-size:18px;font-weight:700;color:#0F172A">New access request</p>
    <table cellpadding="0" cellspacing="0" border="0" style="width:100%;margin-bottom:24px">
      <tr><td style="padding:6px 0;color:#64748B;font-size:14px;width:120px">Requested</td><td style="padding:6px 0;color:#0F172A;font-size:14px;font-weight:600">{{itemName}} ({{itemKind}})</td></tr>
      <tr><td style="padding:6px 0;color:#64748B;font-size:14px">Person</td><td style="padding:6px 0;color:#0F172A;font-size:14px">{{requesterName}}, {{requesterEmail}}</td></tr>
      <tr><td style="padding:6px 0;color:#64748B;font-size:14px">Account</td><td style="padding:6px 0;color:#0F172A;font-size:14px">{{accountName}}</td></tr>
      <tr><td style="padding:6px 0;color:#64748B;font-size:14px">Why it's locked</td><td style="padding:6px 0;color:#0F172A;font-size:14px">{{reason}}</td></tr>
      <tr><td style="padding:6px 0;color:#64748B;font-size:14px;vertical-align:top">They asked</td><td style="padding:6px 0;color:#0F172A;font-size:14px">{{userQuery}}</td></tr>
    </table>
    <table cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px">
      <tr><td style="border-radius:8px;background:#059669"><a href="{{adminUrl}}" style="display:inline-block;padding:13px 28px;background:#059669;color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:15px;font-family:Helvetica,Arial,sans-serif">Review in Admin →</a></td></tr>
    </table>
    <p style="margin:0;line-height:1.7;color:#64748B;font-size:13px">Open Admin → Assistant to grant or dismiss it.</p>`),
  },
  {
    key: 'owner_access_request_fyi',
    name: 'Access Request (FYI to account owner)',
    description: "Sent to the account's owner(s) when a teammate asks the Curio Assistant for access, so they know. Not sent when the owner is the one asking.",
    recipient: '{{ownerEmails}}',
    trigger: 'assistant_access_request',
    trigger_label: 'Curio Assistant access request',
    schedule: 'Immediate',
    send_type: 'automated',
    variables: ['requesterName', 'requesterEmail', 'accountName', 'itemName', 'itemKind', 'ownerEmails'],
    default_subject: '{{requesterName}} requested access to {{itemName}}',
    default_html_body: emailWrap(`
    <p style="margin:0 0 16px;line-height:1.7;color:#0F172A;font-size:15px">Hi,</p>
    <p style="margin:0 0 16px;line-height:1.7;color:#0F172A;font-size:15px">{{requesterName}} ({{requesterEmail}}) on your Curio account asked for access to <strong>{{itemName}}</strong> through the Curio Assistant.</p>
    <p style="margin:0 0 24px;line-height:1.7;color:#0F172A;font-size:15px">No action is needed from you. Curio reviews access requests and will follow up. We're letting you know as the account owner.</p>
    <p style="margin:0;line-height:1.7;color:#0F172A;font-size:15px">Ray Kearney<br>Curio<br><a href="mailto:hello@choosecurio.com" style="color:#059669;text-decoration:none">hello@choosecurio.com</a></p>`),
  },
];
...
async function _send(resend, triggerKey, recipientPattern, subject, html_body, vars) {
  ...
  // Any trigger whose vars include an unsubscribeUrl gets a List-Unsubscribe
  // header for free (one-click unsubscribe in Gmail/Outlook, better
  // deliverability) — currently just cron_weekly_tip, but this stays
  // generic so any future opt-outable send picks it up automatically.
  const headers = vars.unsubscribeUrl ? { 'List-Unsubscribe': `<${vars.unsubscribeUrl}>` } : undefined;

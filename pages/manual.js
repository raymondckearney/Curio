        ...
        <main className="content">
          ...
          <section className="section" id="portal-session-architect">
            ...
            <Card title="Generate Session Materials">Builds every file needed to run the session and bundles them into one <code>.zip</code> download (via <code>pptxgenjs</code>, a second <code>jsPDF</code> pass, and <code>jszip</code>, all client-side):
              <ul>
                <li><strong>Deck (.pptx)</strong> — a cover slide, an agenda overview, then a section-divider slide and a full activity-instruction slide (time/energy/materials, numbered steps, facilitator tips, and the "you'll know it worked when" signal) for every block, closing with a wrap-up slide. Styled off the same brand palette and fonts as the rest of the portal, with a soft accent-colored circle standing in for artwork on each divider slide. Suggested openers and "no one in this orientation is in the room" flags carry over from the on-screen agenda.</li>
                <li><strong>Facilitator guide (.pdf)</strong> — the branded, upgraded version of the Download PDF content: cover, agenda overview, then one page per activity with steps, facilitator tips, and the signal callout.</li>
                <li><strong>Participant handout (.pdf)</strong> — the same cover/agenda/per-activity structure, but steps only: no facilitator tips or signal callout, since those are facilitation notes, not something to hand to the room.</li>
                <li><strong>Activity cards (.pdf)</strong> — one large-type, glanceable card per activity (name, time, energy, one-line purpose, materials), meant to be printed and kept visible in the room during that block rather than read start to finish. No numbered steps by design — that's what the guide/handout are for.</li>
              </ul>
            </Card>

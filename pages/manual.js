      ...
      <div className="shell">
        ...
        <main className="content">
          ...
          <section className="section" id="portal-team">
            ...
            <Card title="Invite a Team Member">Owner only. Enter an email, optional name, and role (Member or Owner). A portal account is created and a "Set Up Your Account" email is sent with a 7-day setup link. The invited user sets their own password. Managers cannot invite or remove members from here — that stays owner-only; a manager's assignment to a team is set by an admin (or, once available, an owner) in the admin panel's Teams section.</Card>
            <Card title="Team Table">An owner sees every portal user on the account; a manager sees only the users on their own team. Columns: name, email, role, assessment status (No Token / Invited / Completed), MindPrint™ profile type, and join date. Owners can remove members (but not themselves) with a confirmation prompt; managers have no remove action.</Card>
            <Card title="Manager role and team scoping">A third role, <code>manager</code>, sits alongside owner and member on <code>client_users</code>, backed by a <code>teams</code> table and a <code>team_id</code> column on both <code>client_users</code> and <code>tokens</code> (assigned in admin Accounts → Edit Account → Teams and Users & Roles). A manager sees the same four tabs as an owner — My Team, Assessment Tokens, Assessment Results, Analytics — but every one of them is filtered server-side to the manager's own <code>team_id</code>, including the token pool they can send from on Assessment Tokens (their one write action) and the stats/recent-assessments shown on their own My Profile dashboard. A manager gets no other owner action: no invite/remove, no license or tier changes, no visibility into other teams or the enterprise-wide unassigned token pool. <code>isTeamAccount</code> (which gates whether these tabs appear at all) is still computed from the whole account's token count, not the manager's own team size, so a small or brand-new team doesn't lose the tabs.</Card>
          </section>

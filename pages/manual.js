      ...
      <div className="shell">
        ...
        <main className="content">
          ...
          <section className="section" id="portal-ai-delegation-guide">
            ...
            <Card title="Not built yet">Collection E (team/meeting tools) is deliberately out of scope — it needs a team context this page doesn't use content from yet. A later phase 3 (a free-text task input with a live AI routing recommendation, rather than this fixed lookup table) is also not built.</Card>
          </section>
          ...
          <section className="section" id="portal-library">
            ...
            <Card title="Communication Field Guides">A sixth, per-profile group below the four support-mode collections: one PDF per MindPrint™ profile (WHY-WHAT, WHY-HOW, WHAT-WHY, WHAT-HOW, HOW-WHY, HOW-WHAT), covering how that profile writes and speaks, how each orientation hears it, and the three adjustments that buy the most understanding. Stored under <code>field-guides/</code> in the same <code>library</code> bucket; rows live in <code>library_items</code> with <code>item_type='field_guide'</code> and a <code>profile</code> value instead of <code>tool_num</code>/<code>collection</code> (both nullable for this row shape). Accent color is by <strong>primary</strong> orientation, not tertiary collection: WHY mint, WHAT blue, HOW amber.
              ...
              Field guides are rendered as web pages at <code>/portal/library/field-guide/[profile]</code> (e.g. <code>/portal/library/field-guide/why-what</code>). Content is stored in <code>lib/fieldGuideContent.js</code> (auto-generated from the source PDFs via <code>/tmp/parse-field-guides.mjs</code>). Each guide page has a <strong>Save as PDF</strong> button using <code>window.print()</code>. The library index "Download Guide" button has been replaced with a "View Guide" link to the web page. Access logic lives in <code>pages/api/portal/library.js</code> and <code>library-file.js</code> as an explicit <code>item_type === 'field_guide'</code> branch alongside the collection-based check, since field guides aren't collection-shaped.
            </Card>
          </section>

          ...
          <section className="section" id="portal-session-architect">
            ...
            <Card title="Generate Session Materials">Builds every file needed to run the session and bundles them into one <code>.zip</code> download (via <code>pptxgenjs</code>, a second <code>jsPDF</code> pass, and <code>jszip</code>, all client-side):
              <ul>
                ...
                <li><strong>Activity cards (.pdf)</strong> — one large-type, glanceable card per activity (name, time, energy, one-line purpose, materials), landscape-oriented for printing and posting in the room during that block rather than read start to finish. No numbered steps by design — that's what the guide/handout are for.</li>
              </ul>
            </Card>
